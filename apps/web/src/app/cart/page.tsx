...
export default function CartPage() {
  ...
  return (
    <div className="mx-auto max-w-[900px] px-4 py-8 sm:px-6" style={{ minHeight: 'calc(100vh - 200px)' }}>
      {/* Page header */}
      ...
      {loading ? (
        <div className="space-y-5">{[1, 2].map((i) => <div key={i} className="h-40 rounded-2xl bg-white animate-pulse" style={{ border: '1px solid #E0E0D8' }} />)}</div>
      ) : isEmpty ? (
        /* Empty state */
        ...
        <>
          ...
          {cart!.sellers.map((seller) => {
            ...
            return (
              <div key={seller.seller_id} className="rounded-2xl bg-white overflow-hidden mb-5" style={{ border: '1px solid #E0E0D8', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
                {/* Seller header — full width */}
                <Link href={`/profile/${seller.seller_id}`} className="flex items-center gap-3 transition-colors hover:bg-[#FAFAF8]" style={{ padding: '14px 20px', borderBottom: '1px solid #E0E0D8' }}>
                  <div className="rounded-full overflow-hidden flex-shrink-0" style={{ width: '36px', height: '36px', backgroundColor: seller.seller_avatar ? undefined : '#1DC690' }}>
                    {seller.seller_avatar ? (
                      <img src={seller.seller_avatar} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <div className="flex items-center justify-center w-full h-full text-white font-bold" style={{ fontSize: '0.8rem' }}>{(seller.seller_name || '?')[0].toUpperCase()}</div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    <span className="truncate" style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '0.9rem', color: '#0D0D0D' }}>{seller.seller_name || 'Seller'}</span>
                    ...
                    {Number(seller.seller_rating) > 0 && <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.78rem', color: '#6B6B6B' }}>⭐ {Number(seller.seller_rating).toFixed(1)}</span>}
                  </div>
                ...
                {/* Two-column body: items left, breakdown right */}
                <div className="flex flex-col lg:flex-row">
                      ...
                      return (
                        <div key={item.id} className="flex items-start gap-4" style={{ padding: '16px 20px', borderBottom: isLast ? 'none' : '1px solid #F4F4F0' }}>
                          ...
                          <Link href={`/listings/${item.listing_id}`} className="flex-shrink-0">
                            <div className="overflow-hidden" style={{ width: '110px', height: '110px', borderRadius: '10px', backgroundColor: '#F4F4F0' }}>
                          ...
                          <div className="flex-1 min-w-0 px-1">
                            <Link href={`/listings/${item.listing_id}`}>
                              <p className="line-clamp-2" style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '0.95rem', color: '#0D0D0D', lineHeight: 1.3 }}>{item.title}</p>
                            </Link>
                  ...
                  {/* Right — per-seller breakdown */}
                  <div className="lg:min-w-[220px] lg:max-w-[240px]" style={{ padding: '20px', backgroundColor: '#FAFAF8', borderLeft: '0', borderTop: '1px solid #E0E0D8' }}>
                    <style>{`@media (min-width: 1024px) { .seller-breakdown { border-left: 1px solid #E0E0D8 !important; border-top: none !important; } }`}</style>
                    <div className="seller-breakdown" style={{ fontFamily: 'var(--font-sans)' }}>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span style={{ fontWeight: 500, fontSize: '0.85rem', color: '#6B6B6B' }}>Item{seller.items.length > 1 ? 's' : ''}</span>
                          <span style={{ fontWeight: 600, fontSize: '0.85rem', color: '#0D0D0D' }}>{formatPrice(sellerItemsTotal)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span style={{ fontWeight: 500, fontSize: '0.85rem', color: '#6B6B6B' }}>Shipping</span>
                          <span style={{ fontWeight: 600, fontSize: '0.85rem', color: '#0D0D0D' }}>{sellerShipping > 0 ? formatPrice(sellerShipping) : 'Free'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-baseline">
                        <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '0.9rem', color: '#0D0D0D' }}>Subtotal</span>
                        <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 800, fontSize: '1.1rem', color: '#1DC690' }}>{formatPrice(sellerSubtotal)}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {/* Grand total card */}
          <div className="rounded-2xl bg-white overflow-hidden" style={{ border: '1px solid #E0E0D8', boxShadow: '0 1px 6px rgba(0,0,0,0.06)', padding: '24px' }}>
            ...
            <div className="flex justify-between items-baseline">
              ...
              <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 800, fontSize: '1.6rem', color: '#1DC690' }}>{formatPrice(summary!.grand_total)}</span>
            </div>

            <p className="mt-2 text-center" style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', color: '#6B6B6B' }}>
            ...
            <button onClick={handleCheckout} className="mt-4 w-full text-white font-bold transition-colors hover:opacity-90" style={{ fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '1rem', height: '54px', backgroundColor: '#1DC690', borderRadius: '12px' }}>
              Proceed to Checkout
            </button>
          </div>
        </>
